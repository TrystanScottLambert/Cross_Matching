...
## In the case that we want to use the ra/dec/vel we need calculate the angular separation. This is the explicit formulism
...
def search_around_point(x,y,z,x_array,y_array,z_array,xy_lim,z_lim,frame='cartesian'):
	idx = np.arange(0,len(x_array))    #making an array representing the positins in the x_array
	...
	if frame=='cartesian':  #checking which frame we are using. 
		...
		os_distances = angsep(x,x_array,y,y_array) #getting the distance in degrees
